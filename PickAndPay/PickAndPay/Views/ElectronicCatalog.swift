                        ...
                        VStack(alignment: .center, spacing: 8) {
                            ForEach(prods[0], id: \.uuid) { prod in
                                NavigationLink(destination:
                                    ElectronicDetail(product: prod, cart: self.$cart, favorites: self.$favorites)
...
struct ElectronicCatalog_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ElectronicCatalog(products: $items, cart: $cart, favorites: $favorites)
    }
}
