                        ...
                        VStack(alignment: .center, spacing: 8) {
                            ForEach(prods[0], id: \.uuid) { prod in
                                NavigationLink(destination:
                                    EssentialsDetail(product: prod, cart: self.$cart, favorites: self.$favorites)
...
struct EssentialsCatalog_Previews: PreviewProvider {
    ...
    static var previews: some View {
        EssentialsCatalog(products: $items, cart: $cart, favorites: $favorites)
    }
}
