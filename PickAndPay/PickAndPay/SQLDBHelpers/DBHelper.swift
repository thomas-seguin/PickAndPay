...
class DBHelper{
    ...
    func createSearchHistoryTable(){
        ...
        let query = "create table if not exists SearchHistory (SearchHistoryId integer primary key autoincrement, UserId text, ProductId integer)"
        if sqlite3_exec(dbpointer, query, nil, nil, nil) != SQLITE_OK{
            let err = String(cString: sqlite3_errmsg(dbpointer)!)
    ...
    func createReviewTable(){
        ...
        let query = "create table if not exists Review (ReviewId integer primary key autoincrement, Body text, Rating double, UserId text, ProductId integer)"
        if sqlite3_exec(dbpointer, query, nil, nil, nil) != SQLITE_OK{
            let err = String(cString: sqlite3_errmsg(dbpointer)!)
